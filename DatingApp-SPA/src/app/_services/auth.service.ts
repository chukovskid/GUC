...
export class AuthService {
  ...
  changeMemberPhoto(photoUrl: string) {
    // ovaa funkcija ke ja vikam za da ja smeni slikata 118
    this.photoUrl.next(photoUrl);
  }

  login(model: any) {
    return this.http
      .post(this.baseUrl + 'login', model) // ova ke vrati TOKEN
      .pipe(
        map((response: any) => {
  ...
  register(user: User) {
    // so ovoj metod prakjam model vo POST na linkot register i si se pravi nov user, voala
  ...
  roleMatch(allowedRole): boolean {
    let isMatch = false;
    // dali nekoj od rolsot na userot e ednakov so rolsot koi dozvoluvaat da vlezes
    const userRoles = this.decodedToken.role as Array<string>; // zemi gi site Roles od JWT tokenot
